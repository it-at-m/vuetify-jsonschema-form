...
export default {
  ...
  computed: {
    ...
    disableAll() {
      if (!this.fullSchema) return
      return this.fullOptions.disableAll
    },
    readonly() {
      if (!this.fullSchema) return
      return this.fullSchema.readOnly
    },
    separator() {
  ...
  methods: {
    ...
    isRequired() {
      ...
      if (this.fullSchema['x-rules']) {
        schemaRequired = this.fullSchema['x-rules'].includes('required')
      }
      return schemaRequired ||
          this.required ||
          this.fullSchema.minLength > 0 ||
          this.fullSchema.minItems > 0
    },
    isReadonly() {
      let schemaReadonly = false
      if (this.fullSchema['readOnly']) {
        schemaReadonly = this.fullSchema['readOnly']
      }
      return schemaReadonly
    },
    renderPropSlots(h) {
      const slots = []
