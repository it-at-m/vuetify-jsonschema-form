...
export default {
  ...
  computed: {
    ...
    readonlyItemSchema() {
      ...
      if (!this.fullOptions.deleteReadOnly) {
  ...
  methods: {
    ...
    renderArrayItemRO(h, item, i) {
      ...
      const renderOptions = {
        props: {
          ...
          options: { ...this.fullOptions, readOnlyArrayItem: true },
    ...
    renderArrayItemEditButton(h, item, i, isCurrentInlineEdit) {
      ...
      return h('v-btn', {
        ...
        attrs: { id: this.fullOptions.idPrefix + this.dashKey + '-' + (isNew ? '-add' : i + '--edit') + '-button', 'aria-label': (isNew ? this.label + ' neues Objekt hinzufügen' : (!isCurrentInlineEdit ? 'Bearbeitungsmodus aktivieren ' + this.label : 'Bearbeitungsmodus deaktivieren ' + this.label)) },
        class: { 'vjsf-array-add-button': true, 'ml-3': isNew },
        props: { icon: !fabIcon, fab: fabIcon, small: isNew, 'x-small': isCurrentInlineEdit, color: isNew ? 'primary' : 'default', depressed: isCurrentInlineEdit } }, [
    ...
    renderArrayItemMenu(h, item, header, isCurrentInlineEdit) {
      ...
      menuItems.map(menuItem => h('v-btn', {
        props: { icon: true, disabled: menuItem.disabled },
        attrs: { 'aria-label': this.label + ' ' + menuItem.title },
        class: 'mr-1',
        on: menuItem.on
      }, [h('v-icon', {}, [menuItem.icon])]))
      )
    },
